...
define([
  ...
  mo.getConfigInfos = function (jimuLayerInfos, configurationLayerInfos, editableOnly, configuredOnly) {
    ...
    //   layerInfo will honor configuration if that layer has configured.
      var configInfos = [];
      var layers = jimuLayerInfos.getLayerInfoArrayOfWebmap();
     var t =  layers.concat(jimuLayerInfos._finalTableInfos)
    array.forEach(t, function (layerInfo) {
      var addLayer = false;
      if (layerInfo.layerObject.type === "Feature Layer" && layerInfo.layerObject.url || layerInfo.isTable) {
        if (layerInfo.layerObject.isEditable && layerInfo.layerObject.isEditable() && editableOnly) {
          addLayer = true;
        }
        else if (editableOnly && editableOnly === false) {
          addLayer = true;
        } else if (layerInfo.isTable) {
            addLayer = true;
        }
