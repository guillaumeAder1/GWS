...
define([
    ...
    function (declare, lang, array, html, on, query, Deferred, editUtils, smartAttributes,
        _TemplatedMixin, _WidgetBase, UndoManager, operationBase, Graphic,
        Query, QueryTask, RelationshipQuery, FeatureLayer, AttributeInspector, domConstruct, domClass, LoadingIndicator, jimuLayerInfos, AttributeInspectorTools) {
        var Clazz = declare([_WidgetBase, _TemplatedMixin], {
            ...
            _init: function () {
                ...
                this.refDomNode = this.editorATI;
            ...
            _createATI: function (operationData) {
                var cloneOperationData = lang.clone(operationData.relatedFeature.attributes);
                ...
                var t = { showDeleteButton: false }
                lang.mixin(tableInfoResult, t);



                if (tableInfoResult) {
                    attributeInspector = this.attrInsp = new Clazz.ATI({
                    ...
                    /**
                     *  Custom code:
                     *      -  add fields to hide, based on the config.json info
                     */

                    this.configFields.forEach(function (field) {
                        if (!field.visible
                            && attributeInspector.layerInfos[0].hideFields.indexOf(field.fieldName) === -1) {
                            attributeInspector.layerInfos[0].hideFields.push(field.fieldName);
                        }
                    }, this);

                    attributeInspector.startup();

                    // create custom delete button to override the default style
                    var deleteButton = domConstruct.create("div", {
                        innerHTML: "Delete",
                        "class": "jimu-btn",
                        style: { "float": "right" }
                    }, attributeInspector.deleteBtn.domNode, "after");
                    //add save button
                    var saveButton = this.saveButton = domConstruct.create("div", {
                        innerHTML: "Save",
                        "class": "jimu-btn disabled"
                    }, attributeInspector.deleteBtn.domNode, "after");

                    var backButton = domConstruct.create("div", {
                        innerHTML: "Back",
                        "class": "jimu-btn",
                ...
                handle = on(backButton, "click", lang.hitch(this, function () {
                    // lang.mixin(operationData.relatedFeature.attributes, cloneOperationData)
                    // this._updateRelatedRecord(operationData).then(lang.hitch(this, this._onPreviouBtnClick));
                ...
                handle = on(deleteButton, "click", lang.hitch(this, lang.hitch(this, this._onDeleteBtnClick, operationData)));
                this._temporaryData.eventHandles.push(handle);

                // custom save button
                handle = on(saveButton, 'click', lang.hitch(this, function (e) {
                    this._isDirty = false;
            ...
            _addNewRelatedRecord: function (operationData) {
                ...
                array.forEach(layerObject.fields, function (field) {
                    ...
                    attr["SchemeID"] = this.currentSchemeID;
                }, this);
            ...
            _deleteRelatedRecord: function (operationData) {
                ...
                relatedLayerObject.applyEdits(null,
                    null, [operationData.relatedFeature],
                    lang.hitch(this, function () {
                        retDef.resolve();
                    }), lang.hitch(this, function () {
                        retDef.reject();
            ...
            _updateRelatedRecord: function (operationData, attributeChangeEvt) {
                ...
                // for 'Back' button, we use the clone of the feature before the changes
                if (attributeChangeEvt) {
                    relatedFeature.attributes[attributeChangeEvt.fieldName] = attributeChangeEvt.fieldValue;
                }

                relatedFeature.attributes["SchemeID"] = this.currentSchemeID
                relatedLayerObject.applyEdits(null, [relatedFeature],
                    null,
                    lang.hitch(this, function () {
                        retDef.resolve();
                    }), lang.hitch(this, function () {
            ...
            showInspector: function (operationData) {
                ...
                this._layerAttr = operationData.destJimuLayerInfo.layerObject;
                ...
                operationData.destJimuLayerInfo.layerObject.selectFeatures(tableQuery,
                    ...
                    lang.hitch(this, function (res) {
                        this.loading.hide();
                        this._isDirty = true;
                        this._validateRequiredFields(this.attrInsp.layerInfos[0].fieldInfos);
                    }));
            ...
            /**
             * Check if any required field value in not empty or white space and trigger validation error if it is. 
             * return the require field list
             * @param {[Array{object}]} layerInfo - Attr inspector layerInfo[array]
             */
            _validateRequiredFields: function (layerInfo) {
                var requiredFields = layerInfo.filter(function (field) {
                    return field.field.nullable === false && field.dijit && !field.field.domain
                });
                requiredFields.forEach(function (field) {
                    field.dijit.setValue(field.dijit.value.trim())
                });
                requiredFields.filter(function (field) {
                    return !field.dijit.validate()
                }).forEach(function (field) {
                    this._triggerError(field);
                }, this);

                return requiredFields.length;
            },
            /**
             * manually trigger the form validation on a field and set state to Error so the field appears in Red on screen
             * @param {object} field - input UI field from attr inspector.layerInfo
             */
            _triggerError: function (field) {
            ...
            /**
             * Check if some fields are not correct and return them in [array] - return false if all correct
             * @param {object} field - Attr inspector layerInfo[Field]
             */
            _validateAllFields: function (fields) {
                var notValidFields = fields.filter(function (field) {
                    return field.dijit
                }).filter(function (field) {
                    return field.dijit.validate && typeof (field.dijit.validate) === 'function' && !this.checkfieldNotEmpty(field.dijit)
                }, this);
                return (notValidFields.length) ? notValidFields : false;
            },
            /**
             * Return trimed value of the field
             * @param {Object} dijit - dijit ui input field
             */
            checkfieldNotEmpty: function (dijit) {
                return dijit.value.trim();
            },
            ...
            _onAttributeChange: function (operationData, evt) {
                /**
                 * Custom code:
                 *      - reassign the new field value to OpreationData.relatedFeature even if not valid
                 *      - force validation field before saving 
                 *      - display error for each incorrect field
                 *      - prevent from saving if at least one field is incorrect
                 */
                operationData.relatedFeature.attributes[evt.fieldName] = evt.fieldValue;
                var fieldNotValid = this._validateAllFields(this.attrInsp.layerInfos[0].fieldInfos);
                if (!fieldNotValid) {
                    domClass.remove(this.saveButton, "disabled");
                } else {
                    fieldNotValid.forEach(function (field) {
                        this._triggerError(field);
                    }, this);
                    domClass.add(this.saveButton, "disabled");
                    return;
            ...
            _onPreviouBtnClick: function () {
                ...
                // force scroll top           
                query(".jimu-container")[0].scrollTop = 0;
            },
